...
def dashboard(request):
	context = {
		...
		'othertrips': Trip.objects.all().exclude(planner=User.objects.get(username=request.session['user']))
	}
...
def submit(request):
	if len(request.POST['destination']) < 0:
		return render(request, 'travels/add.html', {'error': 'No empty values!'})
	...
	try:
		...
		return render(request, 'travels/add.html', {'error': 'Incorrect date format!'})

	if datetime.datetime.strptime(fromDate, "%Y-%m-%d") < datetime.datetime.now():
		return render(request, 'travels/add.html', {'error': 'Future date required!'})
	...
	if datetime.datetime.strptime(fromDate, "%Y-%m-%d") > datetime.datetime.strptime(toDate, "%Y-%m-%d"):
		return render(request, 'travels/add.html', {'error': 'From must be before To!'})
